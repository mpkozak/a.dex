...
export default class Meters extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return this.state.dataRms < -60 ? false : true;
  };
...
// d3 calculation declared
    ...
    // if (dataRms > -60) this.setState(prevState => ({ dataCurve, dataRms, dataPeak }));


    // this.dataCurve = dataCurve;
    // this.dataRms = 20 * Math.log10(Math.sqrt(dataSum / fftSize)) + 20;
    // this.dataPeak = this.dataRms > 15 ? new Date() : this.dataPeak;
  };


  render() {
    console.log('Meters rendered', this.count)
